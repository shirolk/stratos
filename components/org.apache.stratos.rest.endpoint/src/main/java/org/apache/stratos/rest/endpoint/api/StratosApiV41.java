...
public class StratosApiV41 extends AbstractApi {
    ...
    @Consumes("application/json")
    @AuthorizationAction("/permission/admin/manage/add/deploymentPolicy")
    public Response getDeployedApplications()
            throws RestAPIException {
    	// TODO: Fill in with method
        //String policyId = StratosApiV41Utils.getDeployedApplications();
        //URI url = uriInfo.getAbsolutePathBuilder().path(policyId).build();
        return Response.accepted().build();
    }
    ...
    /*
    /deploymentPolicies/{category}/{id}/
    /deploymentPolicies/{category}/{id}/{categoryId}
     
    /deploymentPolicies/{partitionGroup}/{depoymentPolicyId}
    /deploymentPolicies/{partition}/{depoymentPolicyId}
    /deploymentPolicies/{application}/{applicationId} 
